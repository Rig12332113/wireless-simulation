...
int main(void){
    ...
    for (int i = 0; i < 50; i++){
        received_power[i] = 10 * log10f((51.5 * 1.5) * (51.5 * 1.5) / distance[i] / distance[i] / distance[i] / distance[i]);
        fprintf(power, "%.2f %.2f\n", distance[i], 61 + received_power[i] - 30);
    }
    fclose(power);
    //計算各點SINR，考慮外圍一圈影響
    FILE *SINR = fopen("SINR.txt", "wt");
    float noise = 0;
    float base_station[6][2] = {{0, 500}, {0, -500}, {250, 500 / sqrt(3)}, {250, -500 / sqrt(2)}, 
                                {-250, 500 / sqrt(3)}, {-250, - 500 / sqrt(3)}};
    for(int i = 0; i < 50; i++){
        float interfere = 0;
        for (int j = 0; j < 6; j++){
            float far_distance = sqrt((base_station[j][0] - location_x[i]) * (base_station[j][0] - location_x[i]) + 
                                      (base_station[j][1] - location_y[i]) * (base_station[j][1] - location_y[i]));
            interfere += pow(10, 6.1) * (51.5 * 1.5) * (51.5 * 1.5) / far_distance / far_distance / far_distance / far_distance;
        }
        noise = 10 * log10(interfere + (1.38 * 300 * pow(10, -13)));
        fprintf(SINR, "%.2f %.2f\n", distance[i], 61 + received_power[i] - noise);
    }
    fclose(SINR);
    return 0;
}